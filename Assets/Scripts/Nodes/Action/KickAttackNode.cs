...
public class KickAttackNode : Node
{
    ...
    // 생성자: 필요한 인자들을 받도록 수정
    public KickAttackNode(Transform agentTransform, PaladinActuator.AttackType type, string skillName, float cooldownDuration, float animDuration, BT_Aggressive_Paladin btPaladinInstance)
    {
    ...
    public override NodeState Evaluate()
    {
        ...
        {
            // Debug.Log($"KickAttackNode: Another action ({btPaladin.currentActionName}) is in progress. Returning FAILURE.");
        ...
        {
            // Debug.Log($"KickAttackNode: {skillName} - Attack already active. Returning RUNNING.");
        ...
        {
            // Debug.Log($"KickAttackNode: {skillName} - Cooldown not finished. Returning FAILURE.");
            return NodeState.FAILURE;
        }
        ...
        if (!isAttackActive && attackCoroutine == null)
        {
            ...
            Debug.Log($"<color=orange>KickAttackNode: Initiating {skillName} attack.</color>");
        ...
        if (!isAttackActive && attackCoroutine == null)
        {
            Debug.Log($"<color=green>KickAttackNode: {skillName} - Attack completed. Returning SUCCESS.</color>");
            return NodeState.SUCCESS; // 공격 행동 완료
        }

        // 예기치 않은 상황
        Debug.LogWarning($"KickAttackNode: {skillName} - Unexpected state. Returning FAILURE.");
        return NodeState.FAILURE;
    }
    ...
    private IEnumerator AttackCoroutineInternal(float duration)
    {
        ...
        Debug.Log($"<color=yellow>KickAttackNode: {skillName} - Attack duration ended. Setting to inactive.</color>");
